...
//    fetchCache()
  ...
  public func load(type: Reuse,
    ...
    guard let adId = adConfig.getAdId() else {
      print("[MediationAd] [AdManager] Ads don't exist! (\(placement))")
      fail?()
      return
    }
    ...
    adProtocol.config(id: adId, name: adConfig.name)
...
extension AdManager {
  ...
  private func decoding(data: Data, resource: Resource) {
    ...
    do {
      ...
      print("[MediationAd] [AdManager] Invalid (AdMobConfig) format - \(error.localizedDescription)!")
      switch resource {
      case .remote:
