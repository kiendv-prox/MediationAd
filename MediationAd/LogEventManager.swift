...
class LogEventManager: @unchecked Sendable {
    ...
    func log(event: Event) {
        ...
        Task { @MainActor in
            let valid = await isValid(event.name, limit: 40)
            let params = await event.getParameters()
            print("[MediationAd] [LogEventManager]", "[\(valid)]", event.name, params ?? String())
        }
    ...
    func checkFormat(adConfig: AdConfig) {
        ...
        let body: ((AdConfigProtocol) -> Void) = { [weak self] ad in
            ...
            Task {
                let placementValid = await self.isValid(ad.placement, limit: maxCharacter)
                let nameValid = await self.isValid(ad.name, limit: maxCharacter)
                guard !placementValid || !nameValid else {return}
                Task {@MainActor in
                    self.showWarning()
                }
            }
...
extension LogEventManager {
    private func isValid(_ input: String, limit: Int) async -> Bool {
...
enum Event {
    ...
    func getParameters() async -> [String: Any]? {
        switch self {
        ...
        case .adShowCheck(_, _, let viewController),
                .adShowRequest(_, _, let viewController),
                .adShowReady(_, _, let viewController),
                .adShowNoReady(_, _, let viewController),
                .adShowSuccess(_, _, let viewController),
                .adShowHide(_, _, let viewController),
                .adShowClick(_, _, let viewController),
                .adEarnReward(_, _, let viewController),
                .adPayRevenue(_, _, let viewController),
                .adNoRevenue(_, _, let viewController):
            
            var controller: UIViewController?
            if let viewController {
                controller = viewController
            } else if let viewController = AdManager.shared.rootViewController {
                controller = viewController
            } else if let viewController = await UIApplication.topViewController() {
                controller = viewController
            }
            guard let controller else { return nil }
            let screenName = await controller.getScreen()
            return ["screen": screenName]
        case .adShowFail(_, _, let error, let viewController):
            guard let topVC = await UIApplication.topViewController() else { return nil }
            let screenName = await (viewController ?? AdManager.shared.rootViewController ?? topVC).getScreen()
            return [
                "screen": screenName,
