...
//    private var consentConfig: ConsentConfig?
    ...
    public func requestConsentUpdate(completed: @escaping ConsentHandler) {
        ...
        Task { @MainActor in
            do {
                try await UserMessagingPlatform
                    .ConsentForm
                    .presentPrivacyOptionsForm(from: viewController)
                ...
                print("[MediationAd] [ConsentManager] Form error - \(error.localizedDescription)!")
                LogEventManager.shared.log(event: .consentManagerUpdateError)
                completed(.error)
            }
        }
    ...
    private func requestConsentInfoUpdate(params: RequestParameters?) async throws {
        return try await UserMessagingPlatform
            .ConsentInformation
            .shared
            .requestConsentInfoUpdate(with: params)
    }
    ...
    private func presentConsentIfRequired() async throws {
        ...
        return try await UserMessagingPlatform.ConsentForm.loadAndPresentIfRequired(from: controller)
    }
}
